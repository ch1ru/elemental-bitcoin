...
namespace LBitcoin {
    ...
    class HDPrivateKey : PrivateKey {
        ...
        public HDPublicKey XPub(uint index, bool isHardened = false, uint type = 0) {
            ...
            return this.ChildAt(index, isHardened, type).Neuter();
        }
        ...
        public HDPublicKey XPub(HDPath path = null, uint type = 0) {
            ...
            foreach (var level in path.hierarchies_) {
                childKey = childKey.ChildAt(level.child_, level.isHardened_, type);
            }
        ...
        public HDPrivateKey ChildAt(string derPath, uint type = 0) {
            HDPath derivation = new HDPath(derPath);
            return ChildAt(derivation, type);
        }
        ...
        public HDPrivateKey ChildAt(HDPath derPath, uint type = 0) {
        ...
        public HDPrivateKey ChildAt(uint index, bool isHardened = false, uint type = 0) {
            ...
            HDPrivateKey childKey = new HDPrivateKey(
                privateKey, chaincode, depth, fingerprint, childNum, isHardened, this.Testnet_, type);
            return childKey;
        }
