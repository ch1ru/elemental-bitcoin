...
namespace LBitcoin.Networking {
    class NetAddress {
        ...
        public NetAddress(byte[] bytes, bool version = false) {
            if (version) {
                services_ = new NetworkServices(bytes[0..7]);
                ipAddr_ = new IPAddress(bytes[8..23]);
                port_ = BitConverter.ToInt16(bytes[24..25]);
            } else {
                time_ = BitConverter.ToUInt32(bytes[0..3]);
                services_ = new NetworkServices(bytes[4..11]);
                ipAddr_ = new IPAddress(bytes[12..27]);
                port_ = BitConverter.ToInt16(bytes[28..29]);
            }
        }
