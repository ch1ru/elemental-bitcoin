...
namespace LBitcoin.Tests {
    static class ScriptTest {
        ...
        public static void test_address() {
            ...
            byte[] h160 = addr1.getHash(segwit: false, P2PKH: 0);
            Script p2pkh_scriptPubKey = Script.p2pkh(h160);
            Debug.Assert(p2pkh_scriptPubKey.address().ToString() == addr1.ToString());
            BitcoinAddress addr2 = new BitcoinAddress("mrAjisaT4LXL5MzE81sfcDYKU3wqWSvf9q", segwit: false);
            Debug.Assert(p2pkh_scriptPubKey.address(testnet: true).ToString() == addr2.ToString());
            BitcoinAddress addr3 = new BitcoinAddress("3CLoMMyuoDQTPRD3XYZtCvgvkadrAdvdXh", segwit: false);
            h160 = addr3.getHash(segwit: false, P2PKH: 1);
            Script p2sh_scriptpubkey = Script.p2sh(h160);
            Debug.Assert(p2sh_scriptpubkey.address().ToString() == addr3.ToString());
            BitcoinAddress addr4 = new BitcoinAddress("2N3u1R6uwQfuobCqbCgBkpsgBxvr1tZpe7B", segwit: false);
